...
const defaultAbout = `# Welcome to कARMA 👋
...
const AboutMD = () => {
  ...
  useEffect(() => {
    const fetchAbout = async () => {
      try {
        ...
        const savedAbout = res.data.user.about;

        setMarkdown(savedAbout || defaultAbout);
      } catch (err) {
        console.error("Failed to fetch about:", err);
        setMarkdown(defaultAbout);
      }
    };
  ...
  const updateAbout = async () => {
    try {
      ...
      setMarkdown(res.data.profile.about);
