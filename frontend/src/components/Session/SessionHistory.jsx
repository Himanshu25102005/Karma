...
const SessionHistory = ({ compact = false, fillHeight = false }) => {
  ...
  return (
    <>
      <div
        className={`w-full border-1 border-solid border-neutral-800 bg-white/[0.02] rounded-xl p-2 ${compact ? "mt-2 min-h-0 max-h-[320px]" : fillHeight ? "mt-4 flex-1 min-h-0 overflow-hidden" : "mt-4 min-h-[400px]"} flex flex-col`}
      >
        {/* Heading */}
        <div className="shrink-0 h-8 w-full flex justify-between items-center">
          ...
          >
            <svg
              ...
              className="icon icon-tabler icons-tabler-outline icon-tabler-history text-[#5D3FD3]"
            >
        ...
        {/* Toggle (Me<->Friends) */}
        <div
          className={`shrink-0 relative h-12 ${compact ? "w-full max-w-xs" : "w-60"} bg-neutral-900 border border-neutral-800 rounded-xl mt-4 flex p-1`}
        >
          {/* The Animated "Pill" */}
          <motion.div
          ...
          <button
            className={`relative z-10 h-full w-1/2 flex justify-center cursor-target items-center text-xs font-medium transition-colors duration-300 ${
              myActivity ? "text-white" : "text-neutral-500"
            }`}
        ...
        <div
          className={`flex-1 min-h-0 mt-4 flex flex-col gap-2 py-1 pr-1 ${fillHeight || compact ? "overflow-y-auto [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-neutral-800 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-neutral-700" : ""}`}
        >
          {isEmpty ? (
                ...
                return (
                  ...
                  >
                    <div className="h-full w-[58%] flex gap-3 border-r-2 border-neutral-600">
                      <div
                        className="h-full w-[20%] flex justify-center items-center  rounded-xl"
                        style={
                          {
                      ...
                      >
                        <IconClockCode
                          className="h-8 w-8 text-[var(--icon-color)]"
                          style={{
                            "--icon-color": session.projectId?.color
                              ? `${session.projectId.color}`
                              : "#22c55e66",
                          }}
                        />
                      </div>
                    ...
                    <div className="flex-1 rounded-md flex">
                      ...
                      <div className="flex-1 flex justify-center items-center">
                        <IconFileCheck className="text-[#10f11b] h-6 w-6" />
                      </div>
                    </div>
                  </motion.div>
                );
